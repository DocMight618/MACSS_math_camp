...
# Properties of point estimates 
##### Consistency: on average, how does it do? 
*note: efficiency has to do with overall variance across estimators*
...
$$\E(\hat{\pi}_n) = \frac{1}{n} \sum_{i=1}^n \E(X_i) = \pi$$. We can then compare our expected value from the distribution $\pi$, to what we obtained here, to verify they are equal.

They are, so $\hat{\pi}_n$ is unbiased
...
--
We can think of this as: $$\frac{Var(\pi_n)}{n}$$. HOW?? We are adding up all the variances across our random variables and averaging them. 
...
Example: Suppose our null is that people support a six-hour exam. We need to understand if we have sufficient evidence against this. 
