...
* Exams:
  * US (math camp!)
  * US (boot camp!)
  * **OPTIONAL**: [see MPCS website for more information](https://mpcs-internal.cs.uchicago.edu/page/mpcs-placement-exams) 
      * MPCS: math
...
1. Recall the derivative of (natural) $\log(x)$ is one over $x$, and so:   $\int \frac{1}{x} \,dx = \log x + c$
--
...
Answers: 
1. $\int 3x^2 \,dx=\mathbf{x^3 + c}$
1. $\int (2x+1) \,dx = \mathbf{x^2 + x + c}$
1. $\int e^x e^{e^x} \,dx = \mathbf{e^{e^x}+c}$
---
...
```{r defintfig5, fig.cap = "The Riemann Integral as a Sum of Evaluations"}
# generate intervals
...
```{r defintfig2, fig.cap = "The Riemann Integral as a Sum of Evaluations"}
# generate intervals
f3 <- function(x) -15 * (x - 5) + (x - 5)^3 + 50
...
--
Then, we would expect $du=3x^2+3$. BUT! we have: ${\displaystyle \int \frac{\frac{1}{3}du}{u}}$. 
--
