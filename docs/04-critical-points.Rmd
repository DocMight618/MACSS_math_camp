...
* MACSS Orientation 9/22 (expect notices re: schedulng, etc.)
...
```{r rolles-theorem, fig.width=8, fig.height=6, out.width="85%"}
function_plot +
...
fx <- function_plot +
  ...
  geom_abline(slope = -1, intercept = (10 + (1 / 3)), linetype = 4, color = "blue", linewidth = 0.8) +
  labs(y = expression(f(x))) +
  coord_cartesian(xlim = c(0,10)) + coord_cartesian(ylim = c(-1,10)) +
  theme_minimal()#base_size = rcfss::base_size * .7)
...
```{r inflect-deriv0, out.width="55%", fig.height = 2, fig.width=5}
fx <- function_plot +
  stat_function(fun = function(x) (x^3 - 15 * x^2 + 60 * x + 30) / 15, size = 0.5) +
  geom_abline(slope = -1, intercept = (10 + (1 / 3)), linetype = 4, color = "blue", linewidth = 0.8) +
...
```{r inflect-deriv1, out.width="55%", fig.height = 2,  fig.width=5}
fx1 <- function_plot +
  stat_function(fun = function(x) (x^2 / 5) - 2 * x + 4, size = 0.5) +
...
* A critical point when f’(x)=0 implie some kind of concavity.
...
```{r tangent-anim, fig.width=8, fig.height=6, out.width="85%"}
...
```{r base-plot, fig.width=8, fig.height=6, out.width="65%"}
f0 <- function_plot +
...
```{r optims, dependson = "base-plot", fig.width=8, fig.height=6, out.width="65%"}
f0 +
  geom_point(
    data =
      tibble(
...
```{r newton-raphson, fig.width=8, fig.height=6, out.width="75%"}
...
```{r first-func, fig.width=8, fig.height=6, out.width="85%"}
...
```{r first-func-first-guess, dependson = c("newton-raphson", "first-func"), fig.width=8, fig.height=6, out.width="85%"}
# generate iterations
d <- tibble(
  x = newton(fun, x0 = 1),
  y = fun(x)
...
```{r base-plot-two-points, fig.width=8, fig.height=6, out.width="85%"}
f0 <- function_plot +
...
```{r second-func-first-guess, dependson = c("newton-raphson", "second-func"), fig.width=8, fig.height=6, out.width="85%"}
# generate iterations
d <- tibble(
...
```{r second-func-second-guess, dependson = "first-guess", fig.width=8, fig.height=6, out.width="85%"}
# generate iterations
d <- tibble(
  x = newton(fun, x0 = -6),
  y = fun(x)
...
function_plot +
  ...
  geom_point(data = d, aes(x = x, y = y, group = seq_along(label))) +
  geom_point(data = d, aes(x = x, y = y), color = "red") +
  coord_cartesian(xlim = c(-4, 4)) + coord_cartesian(ylim = c(-40, 40)) +
  labs(
    title = expression(x^3 -6*x-2),
    y = expression(f(x)),
    subtitle = "Iteration #{frame_along}"
...
# Newton Raphson: AND BEYOND!

* Just like we were doing, we can also use it to find max / min. 

* It's similar but different: $$x_0 - \frac{f^{'}(x_0)}{f^{''}(x_0)}$$

--

Try it: approximate roots of derivative using the above, starting at 2. 

--
Approximation results: 1.625, 1.428, 1.414
