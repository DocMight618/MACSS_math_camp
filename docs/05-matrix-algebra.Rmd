...
* Library presentation @ 11:30
* TYPOS FIXED IN HERE! SORRY!
...
* Linear independence: cannot get from one vector to another through transformation. Another way to consider is if you have to go *beyond the space (span)* of the set to include an additional vector. 
  * Can think of as multiplication by a scalar, but technically, combining each vector, multiplied by a scalar, gives us zero. (e.g. $\alpha_1*v_1 + \alpha_2*v_2 + ... + \alpha_n*v_n=0$ where at least one scalar is non-zero)
...
* **Step 2:** We leave our first equation $\mathbf{h}$ as-is. We then take our second equation and calculate: $j-h/2$ and replace that result in the middle row. for the third row, we take $k-h$.
...
* **Step 3**: Now, we continue to work, trying to solve for any of our three variables (three variables, three functions). We see that if we add $j+k$, we can get to $\frac{1}{2} \gamma = 0$. This means that $\gamma = 0$. Subbing this into the middle row, $\beta$ must also be 0. Which means that ... $\alpha = 0$ as well. Therefore, these equations (and matrices) are linearly independent as the only possible solution that satisfies our requirements is for all variables to be zero (trivial). 
...
This is conceptually a bit hard as I find it really unintuitive (maybe it's not for you!). 
...
\\&= \begin{pmatrix}1 \times 1 + 0 \times 3 & 1 \times 2 + 0 \times 4 \\
...
* Order of matrix: sometimes you'll see subscripts referring to a matrix's order -- this is particularly true for square matrices. E.g:
  * $I_2$ is an identity matrix (and therefore a square matrix) with two rows and two columns
